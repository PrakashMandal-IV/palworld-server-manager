...
const ROUTE_KINDS = [...EVENT_KINDS, "chat"];
const MAX_HOOKS = 10;
...
function normalizeDiscord(world) {
  ...
  const hooks = url ? [{ id: "default", name: "Default", url }] : [];
  const ev = parseNotifyEvents(world && world.notify_events);
  const routes = {};
  for (const k of EVENT_KINDS) routes[k] = (url && ev[k] !== false) ? "default" : "";
...
module.exports = { EVENT_KINDS, ROUTE_KINDS, MAX_HOOKS, normalizeDiscord, webhookFor };
