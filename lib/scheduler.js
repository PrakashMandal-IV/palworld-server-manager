...
// Deliver any scheduled broadcasts whose time has arrived, then remove them
// (one-shot). Delivery is a best-effort REST announce; if the world is stopped we
// still drop the entry so a missed window doesn't fire late on next boot.
async function fireDueBroadcasts(now) {
  for (const b of dbm.dueBroadcasts(now)) {
    const w = dbm.getWorld(b.world_id);
    try {
      if (w && sup.isRunning(b.world_id) && w.rest_api_enabled) {
        await rest.announce(w, b.message);
        dbm.logEvent(b.world_id, "broadcast", `Sent scheduled broadcast: ${b.message}`);
      } else {
        dbm.logEvent(b.world_id, "broadcast", `Skipped scheduled broadcast (server offline): ${b.message}`);
      }
    } catch (e) {
      dbm.logEvent(b.world_id, "broadcast", `Scheduled broadcast failed: ${e.message}`);
    } finally {
      dbm.deleteBroadcast(b.id); // fired → remove
    }
  }
}
...
async function scheduledRestart(worldId) {
  ...
  const { finalWaittime } = await warn.runPreShutdownWarning(worldId, sup.isRunning);
