...
async function updateWorld(worldId, onLog = () => {}, jobId = null, opts = {}) {
  ...
  try {
    ...
    // Only announce an "updated" when the build id actually changed. A scheduled/auto
    // update that finds nothing new still restarts the server, so say that instead of
    // posting the same "updated to build X" every night (issue #33). If we never knew the
    // old build, we can't tell — fall back to the plain "updated" wording.
