...
const g = globalThis;
if (!g.__PAL_SCHED) g.__PAL_SCHED = { timer: null, bcastTimer: null, bcastBusy: false, updating: new Set(), joinTimers: new Set() };
const ST = g.__PAL_SCHED;
// The global survives module reloads, so an object created before joinTimers existed
// can still be in play here.
...
// Steam on the minute tick, but no more often than this — the build id moves on the
// order of days, and a failed call (offline) just leaves the state unknown until the
// next window rather than nagging.
...
async function maybeAutoCheckUpdates(now = Date.now()) {
  ...
  if (now - last < AUTO_CHECK_MS) return;
...
module.exports = { ensureScheduler, tick, checkUpdates, maybeAutoCheckUpdates, updateWorld, updateAll, startUpdateJob, fireJoinSchedules };
