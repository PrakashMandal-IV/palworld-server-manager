...
function migrate(d) {
  ...
  if (!cols.includes("legacy_perf_flags")) {
    d.exec("ALTER TABLE worlds ADD COLUMN legacy_perf_flags INTEGER NOT NULL DEFAULT 1");
  }
  ...
  if (!cols.includes("discord_bot")) {
    d.exec("ALTER TABLE worlds ADD COLUMN discord_bot TEXT");
  }
...
function updateWorld(id, patch) {
  ...
  db().prepare(`UPDATE worlds SET
    ...
    warn_message=@warn_message, legacy_perf_flags=@legacy_perf_flags
