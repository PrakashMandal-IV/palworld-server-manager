...
function updateBroadcast(id, patch) {
  ...
  if (!cur) return null;
  const merged = { ...cur, ...patch };
  db().prepare("UPDATE broadcasts SET message=@message, fire_at=@fire_at WHERE id=@id").run(merged);
  return db().prepare("SELECT * FROM broadcasts WHERE id=?").get(id);
}
function deleteBroadcast(id) {
  db().prepare("DELETE FROM broadcasts WHERE id=?").run(id);
}
// All broadcasts across every world whose time has come.
function dueBroadcasts(now) {
  return db().prepare("SELECT * FROM broadcasts WHERE fire_at<=? ORDER BY fire_at ASC").all(now);
}
...
module.exports = {
  ...
  listBroadcasts, insertBroadcast, updateBroadcast, deleteBroadcast, dueBroadcasts,
