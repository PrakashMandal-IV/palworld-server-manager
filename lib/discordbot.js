...
function statusCard(world, { online, days, uptime, players, who, note }) {
  const embed = {
  ...
  if (players != null) {
    // Three across on one row: the numbers people came for, readable at a glance.
    embed.fields = [
      { name: "In-game day", value: String(days ?? "—"), inline: true },
    ...
    if (who) embed.fields.push({ name: "Who's on", value: who.slice(0, 1024), inline: false });
  }
...
async function runCommand(name, worldId, interaction) {
  ...
  if (name === "status") {
    if (!sup.isAlive(worldId)) {
      return { embeds: [statusCard(world, { online: false })], detail: "Offline" };
    }
    ...
    if (!world.rest_api_enabled) {
      return {
        embeds: [statusCard(world, { online: true, note: "The in-game day, uptime and player count need this world's REST API, which is switched off. Turn it on in the app under the world's Settings." })],
    ...
    if (!m && !p) {
      ...
      return {
        embeds: [statusCard(world, { online: true, note: "Still starting up — it isn't answering yet. Give it a moment." })],
...
async function handleInteraction(worldId, interaction) {
  ...
  try {
    ...
    const payload = typeof out === "string" ? { content: out } : { content: out.content, embeds: out.embeds };
