...
async function guardTick() {
  ...
  for (const w of worlds) {
    ...
    if (w.status === "running") {
      ...
      // process alive but API frozen for an extended period → treat as hang. A single
      // failed probe is NOT proof of a hang — a healthy server can miss one under load or
      // hit a transient socket error — so we only act after several consecutive failures,
      // log the actual reason each time (not swallow it), and do a final generous-timeout
      // confirmation probe before disrupting a possibly-healthy server (issue #35).
      if (w.rest_api_enabled) {
        const h = await rest.health(w, 8000).catch((e) => ({ ok: false, reason: e && e.message }));
        if (!h.ok) {
          const hangs = (S.__hang ||= new Map());
          const n = (hangs.get(w.world_id) || 0) + 1;
          hangs.set(w.world_id, n);
          dbm.logEvent(w.world_id, "guardian", `Health check failed (${n}/6): ${h.reason || "no response"}`);
          if (n >= 6) { // ~2 min of consecutive failures
            hangs.set(w.world_id, 0);
            // One more probe with a generous timeout: a transient stall shouldn't cost a
            // restart, so only force one when the API is still unresponsive here.
            const confirm = await rest.health(w, 12000).catch((e) => ({ ok: false, reason: e && e.message }));
            if (confirm.ok) {
              dbm.logEvent(w.world_id, "guardian", "API responded on confirmation — not restarting");
            } else {
              dbm.logEvent(w.world_id, "guardian", `API unresponsive — force restarting (hang): ${confirm.reason || "no response"}`);
