...
async function stopWorld(worldId, { graceful = true, waittime = 15 } = {}) {
  ...
  try {
    intendedIni = fs.readFileSync(ini.settingsIniPath(world.install_dir, world.platform), "utf8");
  } catch {}

  if (graceful && world.rest_api_enabled) {
    try {
      try {
        await rest.save(world);
        pushLog(worldId, "World save requested before shutdown");
        await new Promise((r) => setTimeout(r, 3000));
      } catch (e) {
        pushLog(worldId, `Pre-shutdown save failed: ${e.message || e}`);
      }
