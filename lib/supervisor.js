...
function serverBinary(installDir, { hidden = true } = {}) {
  if (os.platform() !== "win32") return path.join(installDir, "PalServer.sh");
  // Fall back to the launcher if this install doesn't have the shipping binary where we
  // expect it — a console window is better than refusing to start.
  if (hidden) {
    const direct = shippingBinary(installDir);
    if (fs.existsSync(direct)) return direct;
  }
  return path.join(installDir, "PalServer.exe");
}
...
async function startWorld(worldId) {
  ...
  const hidden = dbm.getSetting("hideConsoleWindow", true) !== false;
  const bin = serverBinary(world.install_dir, { hidden });
