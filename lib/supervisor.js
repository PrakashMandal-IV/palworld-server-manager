...
function serverBinary(world) {
  const plat = world.platform || hostPlatform();
  return plat === "windows"
  ? path.join(world.install_dir, "PalServer.exe")
  : path.join(world.install_dir, "PalServer.sh");
}
...
function checkWineAvailable(wineBin) {
  const r = spawnSync(wineBin, ["--version"], { timeout: 5000 });
  return !r.error;
}
...
async function startWorld(worldId) {
  ...
  if (world.platform && world.platform !== hostPlat) {
    throw new Error(
      `This world was provisioned for ${world.platform}, but this machine runs ${hostPlat}. ` +
      `Copy the install folder to a ${world.platform} machine to run it, or re-provision for ${hostPlat}.`
    );
  }
  ...
  pushLog(worldId, wine
    ? `Starting via Wine (${wineBin}, prefix ${winePrefix}): ${path.basename(bin)} ${args.join(" ")}`
  ...
  const child = wine
    ? spawn(wineBin, [bin, ...args], spawnOpts)
