...
function buildArgs(world) {
  const args = [
    ...
    // -useperfthreads / -NoAsyncLoadingThread / -UseMultithreadForDS used to be
    // hardcoded here. Post-1.0 these can hurt performance on some setups rather
    // than help, so they're no longer forced — add any of them back per-world
    // via the "Extra Launch arguments" field if you want them.
  ];
